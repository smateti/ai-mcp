 ...
 * eliminating inconsistent responses (sometimes JSON, sometimes text).
 *
 * Usage: pass as the "grammar" field in the llama.cpp API request body.
 * The prompt still needs to describe WHAT fields/values to produce;
 * the grammar only constrains the structural format.
 */
public final class Grammars {
    ...
    /**
     * Forces output to be a valid JSON array: [{"key": "value"}, ...]
     * Used for Q&A generation and batch operations.
     */
    public static final String JSON_ARRAY = """
            root ::= "[" ws array-values? ws "]"
            array-values ::= value (ws "," ws value)*
            value ::= string | number | json-object | array | "true" | "false" | "null"
            json-object ::= "{" ws members? ws "}"
            members ::= pair (ws "," ws pair)*

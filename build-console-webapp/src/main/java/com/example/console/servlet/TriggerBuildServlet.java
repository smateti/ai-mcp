...
public class TriggerBuildServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            JsonObject result = client.triggerBuild(appName, environment);
            long pipelineId = result.getJsonNumber("pipelineId").longValue();
            LOG.info("Triggered pipeline " + pipelineId + " for " + appName + "/" + environment);
            req.getSession().setAttribute("flashMessage",
                    "Pipeline " + pipelineId + " triggered for " + appName + " (" + environment + ")");
        } catch (Exception e) {

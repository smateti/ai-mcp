...
public class RerankerService {
    ...
    public RerankerService(
            @Value("${naag.rag.rerank.enabled:false}") boolean enabled,
            @Value("${naag.rag.rerank.provider:local}") String provider,
            @Value("${naag.rag.rerank.base-url:http://localhost:8001}") String baseUrl,
            @Value("${naag.rag.rerank.model:bge-reranker-base}") String model,
            @Value("${naag.rag.rerank.api-key:}") String apiKey,
            @Value("${naag.rag.rerank.candidate-count:50}") int candidateCount,
            @Value("${naag.rag.rerank.min-score:0.0}") double minScore
    ) {

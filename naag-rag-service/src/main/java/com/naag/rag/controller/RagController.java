...
import com.naag.rag.crag.CragService;
import com.naag.rag.crag.CragService.CragQueryResult;
import com.naag.rag.crag.RetrievalEvaluator;
        ...
        new Thread(() -> {
            try {
                ...
                List<SourceMetadata> sourceMetadataList = sources.stream()
                        .map(s -> new SourceMetadata(s.docId(), s.chunkIndex(), s.relevanceScore(), s.text()))
                        .collect(Collectors.toList());

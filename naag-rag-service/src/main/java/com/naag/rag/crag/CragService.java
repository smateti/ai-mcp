package com.naag.rag.crag;

import com.naag.rag.crag.RetrievalEvaluator.ConfidenceCategory;
import com.naag.rag.crag.RetrievalEvaluator.EvaluationResult;
import com.naag.rag.llm.ChatClient;
import com.naag.rag.metrics.RagMetrics;
import com.naag.rag.service.RagService;
import com.naag.rag.service.RagService.QueryResult;
import com.naag.rag.service.RagService.SourceChunk;
...
public class CragService {
    ...
    public CragService(
            ...
            RagMetrics metrics,
            @Value("${naag.rag.crag.enabled:true}") boolean enabled,
            @Value("${naag.rag.crag.query-expansion-enabled:true}") boolean queryExpansionEnabled,
            @Value("${naag.rag.crag.knowledge-refinement-enabled:true}") boolean knowledgeRefinementEnabled,
            @Value("${naag.rag.crag.max-retry-attempts:2}") int maxRetryAttempts,
            @Value("${naag.rag.crag.min-relevance-for-answer:0.7}") double minRelevanceForAnswer
    ) {

...
│                                                                                 │
│   ┌─────────────────┐        ┌─────────────────┐        ┌─────────────────┐    │
│   │  naagi-chat-app  │        │ naagi-category-  │        │     Ollama      │    │
...
│            ▼                          ▼                           ▼            │
│   ┌────────────────┐       ┌──────────────────┐       ┌────────────────┐       │
│   │ naagi-tool-     │       │ naagi-mcp-gateway │       │ naagi-rag-      │       │
...
|---------|------|---------|
| naagi-rag-service | 8080 | RAG operations, FAQ management, document storage, embeddings |
| naagi-tool-registry | 8081 | Tool metadata, OpenAPI registration |
| naagi-mcp-gateway | 8082 | MCP protocol, tool execution |
| naagi-utility-tools | 8083 | Built-in tools (add, echo, time) |
| naagi-category-admin | 8085 | Admin UI, category management, parameter overrides |
| naagi-ai-orchestrator | 8086 | LLM routing, tool selection, streaming responses |
| naagi-chat-app | 8087 | User chat interface |
...
│   ▼                                                                             │
│  ┌─────────────────────┐                                                        │
│  │   naagi-chat-app     │  POST /api/orchestrator/stream                         │
│  └──────────┬──────────┘                                                        │
│             │                                                                   │
│             ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐       │
...
|---------|-------------|---------|
| **RAG Query** | Semantic search over documents with LLM answer generation | naagi-rag-service |
| **FAQ Cache** | Pre-approved Q&A pairs for instant responses | naagi-rag-service |
| **Reranking** | Two-stage retrieval with cross-encoder for higher precision | naagi-rag-service |
| **CRAG** | Corrective RAG - evaluates confidence and applies fallback strategies | naagi-rag-service |
| **Tool Execution** | Execute registered tools via MCP protocol | naagi-mcp-gateway |
| **Category Management** | Organize tools and documents by category | naagi-category-admin |
| **Parameter Overrides** | Lock tool parameters per category | naagi-category-admin |
| **User Question Analytics** | Track and analyze user questions | naagi-rag-service |
| **Document Preview** | Q&A validation before adding to RAG | naagi-rag-service |
| **Streaming Responses** | Real-time SSE streaming for chat | naagi-ai-orchestrator |
...
|------------|---------|-------------------|
| `naagi_documents` | RAG document chunks | docId, categoryId, text, chunkIndex |
| `naagi_faq` | Approved FAQ pairs | faqId, question, answer, categoryId |
| `naagi_user_questions` | User question tracking | question, categoryId, frequency, lastAsked |
...
spring:
  application:
    name: naagi-service-name

  # H2 Database
  datasource:
    url: jdbc:h2:file:./data/naagi-db;AUTO_SERVER=TRUE
...
naag:
  ...
  user-questions:
    enabled: true
    collection: naagi_user_questions

...
│  ┌────────────────────────┐    ┌─────────────────────────────┐  │
│  │   naagi-chat-app        │    │   naagi-category-admin       │  │
│  │   (Port 8087)          │    │   (Port 8085)               │  │
...
│                     Intelligence Layer                           │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │              naagi-ai-orchestrator (Port 8086)              │ │
...
│ ┌───────────────────────┐ │         │ ┌───────────────────────┐ │
│ │ naagi-mcp-gateway      │ │         │ │ naagi-rag-service      │ │
│ │ (Port 8082)           │ │         │ │ (Port 8080)           │ │
...
│ └───────────────────────┘ │         │ ┌───────────────────────┐ │
│ ┌───────────────────────┐ │         │ │ LLM Server            │ │
│ │ naagi-tool-registry    │ │         │ │ llama.cpp (8000)      │ │
...
3. **Naming Convention**: `naagi-*` prefix with descriptive 3-word names
...
- H2 Database (file: `./data/naagi-rag-db`)

---

### 2. naagi-tool-registry (Port 8081)
...
- Thymeleaf (admin UI)
- H2 Database (file: `./data/naagi-tool-registry`)
...
**Flow**:
1. Loads tool definitions from naagi-tool-registry (8081)
2. Receives tool execution requests
3. Routes to naagi-utility-tools (8083) for built-in tools
4. Routes to external registered endpoints for other tools

---

### 4. naagi-utility-tools (Port 8083)
...
**Startup Behavior**: Auto-registers itself with naagi-tool-registry on startup
...
**Delegates To**:
- naagi-rag-service for document parsing
- naagi-tool-registry for tool management

---

### 6. naagi-ai-orchestrator (Port 8086)
...
- naagi-rag-service (8080) - Direct RAG queries
- naagi-tool-registry (8081) - Tool schemas
- naagi-mcp-gateway (8082) - Tool execution
- naagi-category-admin (8085) - Category configuration

---

### 7. naagi-chat-app (Port 8087)
...
```
User Input (naagi-chat-app:8087)
        │
        ▼
POST /api/orchestrate (naagi-ai-orchestrator:8086)
        │
        ├──► Intent Detection (LLM)
        │
        ├──► If TOOL_CALL:
        │    │
        │    ├──► Get tool schema (naagi-tool-registry:8081)
        │    ├──► Extract parameters (LLM)
        │    ├──► Execute tool (naagi-mcp-gateway:8082)
        │    │         │
        │    │         └──► Route to tool endpoint
        │    │              (naagi-utility-tools:8083 or external)
        │    │
        │    └──► Generate response (LLM)
        │
        ├──► If RAG_QUERY:
        │    │
        │    └──► Query documents (naagi-rag-service:8080)
        │              │
...
```
Admin Upload (naagi-category-admin:8085)
        │
        ▼
POST /api/documents/upload (naagi-rag-service:8080)
        │
...
|---------|------|---------------|
| naagi-rag-service | 8080 | ./data/naagi-rag-db |
| naagi-tool-registry | 8081 | ./data/naagi-tool-registry |
| naagi-mcp-gateway | 8082 | - |
| naagi-utility-tools | 8083 | - |
| naagi-category-admin | 8085 | - |
| naagi-ai-orchestrator | 8086 | - |
| naagi-chat-app | 8087 | - |
| Qdrant | 6333 | - |
...
|----------------|-----------------|-------|
| full-rag-springboot-sync | naagi-rag-service | Spring Boot 3.3.2 → Java 21 |
| mcp-spring-boot-server | naagi-mcp-gateway | Renamed, upgraded |
| mcp-chat-app | naagi-chat-app | Renamed, upgraded |
| dynamic-tool-registry | naagi-tool-registry | Renamed, upgraded |
| mcp-category-admin | naagi-category-admin | Renamed, upgraded |
| mcp-user-chat | naagi-chat-app | Merged functionality |
...
1. Create endpoint in naagi-utility-tools (or external service)
2. Register via naagi-category-admin UI or API:
   ```json
   POST /api/tools
   {
     "name": "my-tool",
...
naagi-rag-service        # 8080 - No dependencies
naagi-tool-registry      # 8081 - No dependencies
naagi-mcp-gateway        # 8082 - Depends on tool-registry
naagi-utility-tools      # 8083 - Depends on tool-registry
naagi-category-admin     # 8085 - Depends on rag-service, tool-registry
naagi-ai-orchestrator    # 8086 - Depends on all core services
naagi-chat-app           # 8087 - Depends on orchestrator
```
...
| Script | Purpose |
|--------|---------|
| start-naagi.ps1 | PowerShell - Start all services |
| start-naagi.bat | Batch wrapper |
| status-naagi.bat | Check service status |
| stop-naagi.bat | Stop all services |
